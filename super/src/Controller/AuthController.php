...
class AuthController extends Controller
{
    public function doLogin()
    {
        ...
        $result = $mahasiswa->getPegawai($nip);

        if ($result == null) {
            Helper::dumpToLog("Super Admin $nip tidak ditemukan");
            session_start();
            $_SESSION['Error'] = "Super Admin $nip tidak ditemukan";
            header("Location: /");
        } else {
            Helper::dumpToLog(json_encode($result));
            if ($result[0]->password == Helper::encrypt($password) && strtolower($result[0]->role) == 'super') {
                Helper::dumpToLog("success login Super Admin $nip");
                $cookieValue = [];
                ...
                Helper::dumpToLog("gagal login Super Admin $nip");
                session_start();
